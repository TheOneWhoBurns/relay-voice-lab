...
try {
  ...
  assert.equal(await page.locator('#language').inputValue(),'es');assert.ok((await page.locator('#greeting').inputValue()).startsWith('Hola'));assert.ok(!(await page.locator('#greeting').inputValue()).includes('treinta'));
  await page.getByRole('tab',{name:'Bookings',exact:true}).click();await page.locator('.slot').first().waitFor();await page.getByRole('tab',{name:'Conversation',exact:true}).click();
